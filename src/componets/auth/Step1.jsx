...
export default function Step1() {
  ...
  const { username, email, communityName, facebook, twitter, discord, desc } =
    formData;
  ...
  const validate = (e) => {
    ...
    !username
      ? (errorObj.username = "Field is required")
      : username.length > 30 || username.length < 3
      ? (errorObj.username = "Field must be within 3-to-30 letters")
      : regexUsername.test(username) == false
      ? (errorObj.username =
          "username must contain only lowercase letters and numbers and must start with a letter")
      : null;
    ...
    !communityName
      ? (errorObj.communityName = "Field is required")
      : communityName.length > 30 || communityName.length < 3
      ? (errorObj.communityName = "Field must be within 3-to-30 letters")
      : "";

    !desc
      ? (errorObj.desc = "Field is required")
      : desc.length > 30 || desc.length < 3
      ? (errorObj.desc = "Field must be within 3-to-30 letters")
      : "";

    desc.length > 100
      ? (errorObj.desc = "Field must be less than 100 characters")
      : null;
        ...
        <form>
          <div className={styles.input}>
            ...
            <div className={styles.inpSocials}>
              ...
              />
              {FormErrors.facebook && (
                <div className={styles.error}>{FormErrors.facebook}</div>
              )}
              <input
                type="text"
              ...
              />
              {FormErrors.twitter && (
                <div className={styles.error}>{FormErrors.twitter}</div>
              )}
              <input
                type="text"
              ...
              />
              {FormErrors.discord && (
                <div className={styles.error}>{FormErrors.discord}</div>
              )}
            </div>
            <div className={styles.inp}>
              ...
              {FormErrors.communityName && (
                <div className={styles.error}>{FormErrors.communityName}</div>
              )}
            </div>
            <div className={styles.inp}>
              ...
              ></textarea>
              {FormErrors.desc && (
                <div className={styles.error}>{FormErrors.desc}</div>
              )}
            </div>
          </div>
          <div className={styles.btn} onClick={validate}>
